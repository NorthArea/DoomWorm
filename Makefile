# BroomWorm developer shortcuts. Everything runs through uv; nothing is installed globally.
...
demo-0: ## Stage 0: three-neuron chain
	$(UV) python -m broomworm.experiments.three_neurons

demo-1: ## Stage 1: obstacle avoidance (PNG in runs/)
	$(UV) python -m broomworm.experiments.obstacle_agent --plot

demo-2: ## Stage 2: food and hunger (PNG in runs/)
	$(UV) python -m broomworm.experiments.food_agent --plot

demo-3: ## Stage 3: reward comparison of three brains
	$(UV) python -m broomworm.experiments.reward_demo

demo-5: ## Stage 5: connectome statistics
	$(UV) python -m broomworm.experiments.connectome_stats

demo-6: ## Stage 6: debug screen GIF (runs/stage6_debug.gif)
	$(UV) python -m broomworm.experiments.debug_screen_demo

demo-7: ## Stage 7: sensory mapping and per-channel responders
	$(UV) python -m broomworm.experiments.sensory_mapping_demo

demo-8: ## Stage 8: motor mapping, wheel commands per channel
	$(UV) python -m broomworm.experiments.motor_mapping_demo

demo-9: ## Stage 9: untrained worm drives the 2D agent (PNG + GIF + JSONL log)
	$(UV) python -m broomworm.experiments.worm_agent --seed $(SEED) --plot --gif

demo-12: ## Stage 12: brain trained on random maps, played on an unseen random map
	$(UV) broomworm play --brain runs/worm_evolved_random.json --seed $(SEED) --plot

demo-13: ## Stage 13: the same brain sent to a target ("come to X") instead of food
	$(UV) broomworm play --brain runs/worm_evolved_random.json --task target --seed $(SEED) --plot

demo-16: ## Stage 16: vacuum task (dirt, dock, battery) on an unseen apartment, stage-12 brain
	$(UV) broomworm play --brain runs/worm_evolved_random.json --maps apartment --task clean --seed $(SEED) --steps 800 --plot

demo-15: ## Stage 15: a brain on an unseen apartment map (rooms, doors, furniture)
	$(UV) broomworm play --brain runs/worm_evolved_random.json --maps apartment --seed $(SEED) --plot

demo-14: ## Stage 14: target task with a danger zone (runs/worm_evolved_danger.json)
	$(UV) broomworm play --brain runs/worm_evolved_danger.json --seed $(SEED) --plot
...
demo-17: ## Stage 17: stage-12 brain under the noisy vacuum sensor suite
	$(UV) broomworm play --brain $(WORM_BRAIN) --maps random --sensors noisy --seed $(SEED) --plot

demo-18: ## Stage 18: benchmark the stage-12 brain alone -> runs/benchmark/
	$(UV) broomworm benchmark --brain $(WORM_BRAIN) --name worm_stage12

demo-19: ## Stage 19: the same brain wrapped in the coverage planner
	$(UV) broomworm benchmark --brain $(WORM_BRAIN) --name worm_stage12 --planner coverage

demo-20: ## Stage 20: needs arbitration (battery > call > clean), worm and scripted driver
	$(UV) broomworm benchmark --brain $(WORM_BRAIN) --name worm_stage12 --planner needs
	$(UV) broomworm benchmark --scripted follower --planner needs

demo-21: ## Stage 21.1: the Roomba-style classical controller, zero learning, on the benchmark
	$(UV) broomworm benchmark --scripted roomba

demo-22: ## Stage 22.1: A2 worm over the simulator link with recording, then the log replayed and compared
	$(UV) broomworm drive --brain $(A2_WORM) --planner needs --seed 3002 --steps 300 --every 50 --record runs/drive/worm_sim_3002.jsonl
	$(UV) broomworm compare-log --log runs/drive/worm_sim_3002.jsonl --out runs/drive/worm_sim_3002_vs_replay.md
	$(UV) broomworm compare-log --log runs/drive/worm_sim_3002.jsonl --sensors noisy --out runs/drive/worm_sim_3002_vs_noisy.md
	printf 'w\nw\nw\nd\nd\nw\nw\n' | $(UV) broomworm drive --teleop --seed 3002 --every 1 --record runs/drive/teleop_3002.jsonl
...
selftest-sim: ## Stage 22.2 rehearsal: self-test, calibration and a room drive on the simulator link
	$(UV) broomworm selftest --sensors car --seed 3001 --out runs/selftest_sim.md
	printf '0.8\n60\n' | $(UV) broomworm calibrate --sensors car --seed 3001 --out runs/calibration_sim.json
	printf 'w\nw\nw\nw\nd\nd\nw\nw\nw\n' | $(UV) broomworm drive --teleop --sensors car --room data/rooms/example_room.json --every 1 --record runs/drive/room_sim.jsonl
	$(UV) broomworm plot-log --log runs/drive/room_sim.jsonl
	$(UV) broomworm compare-log --log runs/drive/room_sim.jsonl

robustness-car: ## Stage 22.1f: sweep the car preset's assumed numbers around the two chosen brains -> runs/robustness/
	$(UV) broomworm robustness --brain docs/brains/a2/worm_from_worm_evolved_random.json --planner needs --sensors car
	$(UV) broomworm robustness --brain docs/brains/car/ppo.json --name ppo_car --planner needs --sensors car
...
benchmark-car: ## Stage 22: every A2 candidate plus the scripted floors on the car sensor preset -> runs/benchmark_car/
	...
	for b in worm worm_from_worm_evolved_random rnn ppo; do \
	  $(UV) broomworm benchmark --brain docs/brains/a2/$$b.json --planner needs --sensors car --out-dir runs/benchmark_car; done
	for b in $(wildcard runs/a2_car/*.json); do \
	  case $$b in *.meta.json) ;; *) $(UV) broomworm benchmark --brain $$b --name $$(basename $$b .json)_car --planner needs --sensors car --out-dir runs/benchmark_car;; esac; done

evolve-car: ## Stage 22: retrain $(CANDIDATE) on the car preset -> runs/a2_car/ (PPO: broomworm ppo --sensors car)
	$(UV) broomworm evolve --candidate $(CANDIDATE) --sensors car --workers 4 --out runs/a2_car/$(CANDIDATE).json
...
train: ## Stage 4: evolve the small network -> $(BRAIN)
	$(UV) broomworm train --out $(BRAIN)

train-worm: ## Stage 10: evolve the connectome weights -> runs/worm_evolved.json
	$(UV) broomworm train --scenario worm --out runs/worm_evolved.json $(TRAIN_ARGS)

train-worm-random: ## Stage 12: evolve the connectome on random maps -> runs/worm_evolved_random.json
	$(UV) broomworm train --scenario worm --maps random --out runs/worm_evolved_random.json $(TRAIN_ARGS)

train-worm-danger: ## Stage 14: continue from the random-map brain on target + danger
	$(UV) broomworm train --scenario worm --maps random --task target --dangers 1 --init-brain runs/worm_evolved_random.json --out runs/worm_evolved_danger.json $(TRAIN_ARGS)

train-worm-apartment: ## Stage 15: continue from the random-map brain on apartment maps
	$(UV) broomworm train --scenario worm --maps apartment --init-brain runs/worm_evolved_random.json --out runs/worm_evolved_apartment.json $(TRAIN_ARGS)

benchmark: ## Stage 18: benchmark $(BRAIN) on apartments/clean/vacuum -> runs/benchmark/
	$(UV) broomworm benchmark --brain $(BRAIN) $(BENCH_ARGS)
...
compare: ## Stage 11: real vs random vs shuffled vs free -> runs/compare/
	$(UV) broomworm compare $(COMPARE_ARGS)

play: ## Replay $(BRAIN) on seed $(SEED) with a plot
	$(UV) broomworm play --brain $(BRAIN) --seed $(SEED) --plot
