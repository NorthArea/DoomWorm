...
| 17 | A1: vacuum sensor suite with noise and delay | `tests/test_sensors.py` | `doomworm play --brain runs/worm_evolved_random.json --maps random --sensors noisy --seed 2002` | done (ideal 0 collisions -> vacuum 171, noisy 104; see `docs/results/a1/stage17_sensors_eval_2026-09-15.md`) |
...
| B2 | Candidates on the mini-Doom benchmark: worm, curriculum worm, random / shuffled controls, rnn, ncp, PPO trained bare (no planner) on `doom4` and `doom6`, 3 seeds, transfer rows on every level, floors and the A2 brains without retraining | `tests/test_bakeoff.py`, `tests/test_doom.py` | `make evolve-doom CANDIDATE=worm LEVEL=doom4`, `make benchmark-doom` | done 2026-09-16: three seeds for every row on both training levels, benchmarked on `doom1`..`doom6` and unchanged in the engine on `vizdoom1`..`vizdoom6`. No trained brain beats the `doomguy` floor; on `doom4` shuffled (−2.1) > random (−4.9) > curriculum (−8.1) > real (−13.7), floor +4.8. Tables `docs/results/b/b2_doom_2026-09-16.md`, axes C9a-C9c in `docs/findings.md`. Lane `scripts/doom_b2_resume.py` (idempotent, resumes an interrupted run) |
