...
| 12 | Re-measure the shooting rows with the reachable trigger and the prey channel: the worm family against its controls and the nets, three seeds, `doom4` and `doom6`, then in the engine unchanged | `tests/test_bakeoff.py` | `uv run scripts/doom_b2d_lane.py` | next |
| 13 | Reorientation: the worm hunts what is in front of it and cannot turn onto a target 60 degrees off the bow. Left to evolution first (see `docs/assumptions.md` 2026-09-17) | | | todo |
