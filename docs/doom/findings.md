...
| 13 the circuit stage 26 said was missing (stage 27) | the reflex measured with no world in the way: hold one side channel steady for 60 ticks and read the intent back. A `face` needs four things — the right **sign**, enough **size**, a turn that **holds** while the signal does, and one that **stops** when the target comes ahead | on `danger` (the sector the floor uses): worm **sign +0.196, hold 0.97, stop 0.030**, and all three seeds agree (+0.158, +0.304, +0.125) | its own shuffle on `danger` is split (−0.473, −0.086, +0.040) and never settles (stop 0.221); the random topology is split too | on `target` the worm's mean is +0.224 but one seed of three disagrees, while the shuffle is consistently **backwards** (−0.364, −0.222, −0.504) | **the wiring does it, and stage 26's note that it "has never been shown" was wrong.** The connectome turns the right way towards a target off the bow, sustains the turn and stops cleanly when it centres. How strongly is another matter: three seeds agreeing is one chance in eight, so read this as the reflex being present and its consistency being suggestive, not established | `scripts/face_probe.py`, per-seed signs printed beside every mean |
| 13 so why it never lands a hit (stage 27) | the same measurement swept over how loud the signal is. The sector channel is 1/distance, so a monster at the gun's range arrives at ~0.17 and one across the room at ~0.05 | worm at 0.05: **+0.007** | at 0.17: **+0.055** | at 1.00: +0.196 — and the hand-written floor turns at 0.6 | **the circuit is there and its gain is out by an order of magnitude where it matters.** At the far end of the room the worm asks for a turn of 0.007; coming round would take longer than the episode. The CMA brain from stage 18 does have the gain — +0.49 at 0.05, +0.72 at 0.17 — and pays for it by never stopping (stop 0.95), so it spins past. Nothing trained so far has both | `scripts/face_probe.py --gain` |
| 13 can any weights do it (stage 27) | train for `face` and nothing else — no world, no reward but the reflex itself, scored at the loudness the game really sends (0.05, 0.17, 0.30), 120 generations of CMA-ES on the 1819 interface synapses | the connectome: 0.001 → **0.735** | its shuffle: −0.061 → 0.499 | **a random topology of the same size: −0.007 → 0.979**, the best of the three | **every topology can express it, and the real one is not the best at it.** So the connectome's advantage here is not capacity. What the row above shows is an inductive bias — trained on the *game*, where nothing ever rewarded facing anything, the real wiring lands the correct sign on all three seeds and neither control does. It is what the anatomy gives you when the reward is indirect, not what it makes possible | `scripts/face_probe.py --train` |
