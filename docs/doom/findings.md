...
| 9 the competence map (stage 22) | 3600 searched decisions on `doom4`, six proposals each, grouped by the situation the channels describe | the connectome's own first answer is the one the rollout keeps in **42-51 %** of decisions, against 17 % by chance. Best where its own reflex lives — **51 % with a wall ahead**, and the search adds least there (+0.06) | — | the search adds **twice as much** wherever a monster is involved (+0.13) as in any exit situation (+0.07) | **the architecture is good at what the animal does**: touch and avoid. It is weak at orienting onto a thing, and two thirds of its decisions (2423 of 3600) are spent in exactly that situation — the exit off to one side | `docs/doom/results/competence/doom4.md` |
