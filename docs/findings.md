...
| 2 a harder level (being re-measured) | `doom6` (apartment, two walking monsters, gun) | real −40.1 ± 3.3; curriculum −24.6 ± 1.0 | ncp −21.3 ± 2.3; PPO −33.0 ± 6.4; rnn −46.2 ± 4.0 | −18.8 (curriculum −3.3) | **loss**; the floor takes +27.2 with 1.33 kills and 75 % exits | same |
...
`doom6` and the engine transfer are still running in the same lane.

## What the map says so far

- On the task it was trained for, the worm now ties PPO and its own controls
  and beats the same-size net, after two mapping defects were fixed. Nothing
  trained has beaten the hand-written floor yet.
