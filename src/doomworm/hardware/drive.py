...
:func:`doomworm.hardware.compare.replay_in_sim`.
"""
...
def drive(
    ...
    when a :class:`~doomworm.hardware.link.SimLink` reports the episode over.
    """
    rows: list[DriveRow] = []
    controller.reset()
    channels = link.reset()
...
def raw_keys(tty: TextIO | None = None) -> TextIO:
    ...
    close). Not used by tests; ``doomworm drive --teleop`` picks it when stdin is a tty.
    """
    import termios
