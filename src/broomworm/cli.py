...
broomworm train                     evolve weights (stage 4 small network)
broomworm train --scenario worm     evolve the connectome weights (stage 10)
broomworm compare                   real vs random vs shuffled vs free topology (stage 11)
broomworm benchmark --brain X       run any saved brain through the benchmark (stage 18)
broomworm evolve --candidate worm   train a bake-off candidate on the benchmark world (stage 21)
broomworm ppo                       train the PPO candidate (optional rl group, stage 21.4)
broomworm play --brain brain.json   replay a saved brain on a seeded world
broomworm drive --teleop            drive the simulator or the machine, record sensors (stage 22)
broomworm compare-log --log X       replay a drive log in the simulator, compare channels
...
from broomworm import __version__
...
def build_parser() -> argparse.ArgumentParser:
    ...
    from broomworm.experiments.compare_topologies import add_compare_args
    from broomworm.experiments.evolve_small import add_train_args
    from broomworm.experiments.evolve_worm import add_worm_args
...
def run_robustness_cli(args: argparse.Namespace) -> int:
    ...
    from broomworm.environments.sensors import PRESETS, SensorConfig
    from broomworm.episode import BrainLike
    from broomworm.learning import CAR_GRID, BenchmarkConfig, sweep, sweep_table, worst_cells
    ...
    if args.scripted == "follower":
        ...
        from broomworm.candidates import RoombaBrain
...
def run_drive_cli(args: argparse.Namespace) -> int:
    ...
    if args.planner != "none":
        from broomworm.environments.sensors import PRESETS
        from broomworm.layer import PlannerLayer
...
def run_benchmark_cli(args: argparse.Namespace) -> int:
    ...
    from broomworm.episode import BrainLike
    from broomworm.learning import BenchmarkConfig, run_benchmark, save_result, write_leaderboard
...
def run_evolve_cli(args: argparse.Namespace) -> int:
    ...
    from broomworm.candidates import CandidateSpec
...
def run_ppo_cli(args: argparse.Namespace) -> int:
    ...
    from broomworm.learning.rl import PPOConfig, train_ppo
...
def run_stimulate(args: argparse.Namespace) -> int:
    ...
    from broomworm.connectome import build_network, load_cook2019
    from broomworm.visualization import plot_active_subgraph, plot_raster, propagation_tree
...
def run_play(args: argparse.Namespace) -> int:
    ...
    from broomworm.brain import Simulator, load_brain
    from broomworm.episode import Record, run_brain_episode
    from broomworm.experiments.episode import print_trace, render_ascii, run_episode, save_plot
    from broomworm.experiments.evolve_small import SmallFoodScenario
    from broomworm.experiments.worm_agent import WormScenario
    from broomworm.learning import RewardTracker, Scenario
    ...
    if scenario_name == "small_food":
        ...
        from broomworm.candidates import WormBrain
    ...
    from broomworm.experiments.worm_agent import summarise
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "train":
        if args.scenario == "worm":
            from broomworm.experiments.evolve_worm import run_train as run_train_worm

            return run_train_worm(args)
        from broomworm.experiments.evolve_small import run_train

        return run_train(args)
    if args.command == "stimulate":
        return run_stimulate(args)
    if args.command == "compare":
        from broomworm.experiments.compare_topologies import run_compare
