...
Every candidate is a :class:`~broomworm.candidates.base.Brain` that is also
:class:`~broomworm.candidates.base.Trainable`; the training harness only sees the
...
def load_candidate(path: Path | str, **world: Any) -> BrainLike:
    ...
    if kind == "ppo":
        try:
            from broomworm.learning.rl import PPOBrain
...
def build_candidate(spec: CandidateSpec) -> TrainableBrain:
    ...
    if spec.kind == "hybrid":
        ...
        from broomworm.candidates.hybrid import build_hybrid
    ...
    if spec.kind != "worm":
        ...
        from broomworm.connectome.variants import make_variant
