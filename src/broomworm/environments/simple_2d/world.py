...
from broomworm.body import Body, DifferentialDrive, Drive
...
class World:
    ...
    def __init__(
        ...
        *,
        ...
        # Arbitrary-angle solid lines (rooms measured from a real flat may have them).
    ...
    def finished(self) -> bool:
        """True when the episode is over (dead)."""
        return self.dead
    ...
    def step(self, motor_left: float, motor_right: float) -> Observation:
        ...
        return self.drive(Drive.from_wheels(left, right))
    ...
    def _sense_danger(self) -> tuple[float, float, float]:
        """Nearest danger zone (a smell: sensed through walls)."""
