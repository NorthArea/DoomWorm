...
commands in [-1, 1]. Episodes end when the agent is dead
...
from broomworm.environments.sensors import PASSTHROUGH, PRESETS, SensorSuite
...
class BroomwormEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def render(self) -> str:
        ...
        from broomworm.experiments.episode import render_ascii
...
class PlannerWrapper(gym.Wrapper[np.ndarray, np.ndarray, np.ndarray, np.ndarray]):
    ...
    def __init__(self, env: BroomwormEnv, mode: str = "needs") -> None:
        super().__init__(env)
        self.base = env
