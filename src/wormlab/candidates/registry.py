...
Every candidate is a :class:`~wormlab.candidates.base.Brain` that is also
:class:`~wormlab.candidates.base.Trainable`; the training harness only sees the
...
def load_candidate(path: Path | str, **world: Any) -> BrainLike:
    ...
    if kind == "ppo":
        try:
            from wormlab.learning.rl import PPOBrain
...
def build_candidate(spec: CandidateSpec) -> TrainableBrain:
    ...
    if spec.kind == "hybrid":
        ...
        from wormlab.candidates.hybrid import build_hybrid
    ...
    if spec.kind != "worm":
        ...
        from wormlab.connectome.variants import make_variant
