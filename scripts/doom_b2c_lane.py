...
# dependencies = []
# ///
"""Stage B2c: the track-B worm rows retrained with a trigger the connectome can reach.
...
could: shuffling gives the island 399 crossing connections). With the signal now
delivered to RIP, the animal's only door into the pharynx, the worm family is
retrained under the unchanged B2 protocol: doom4 and doom6, three seeds,
...
TRAIN_LOG = ROOT / "runs/doom_rip/train.log"
...
def brain_path(level: str, seed: int, name: str) -> Path:
    return ROOT / f"runs/doom_rip/{level}/seed{seed}/{name}.json"
...
def eval_csv(level: str, seed: int, name: str, eval_map: str) -> Path:
    return ROOT / f"runs/benchmark_doom_rip/train_{level}/seed{seed}/eval_{eval_map}/{name}.csv"
