...
# dependencies = []
# ///
"""Stage 18b: the strongest search this architecture gets, on the smallest genome.
...
OUT = ROOT / "runs/squeeze2"
LOG = OUT / "lane.log"
SEEDS = (0, 1, 2)
ROWS = (
    ("cma", ["--search", "cma", "--init-sigma", "0.2"]),
    ("cma_interface", ["--search", "cma", "--init-sigma", "0.2", "--trainable", "interface"]),
    (
        "cma_interface_gains",
        ["--search", "cma", "--init-sigma", "0.2", "--trainable", "interface", "--tune-gains"],
    ),
)
...
def main() -> int:
    if "--no-detach" not in sys.argv:
        os.setsid()
    started = time.time()
    log(f"=== SQUEEZE2 LANE START {stamp()} === {len(ROWS) * len(SEEDS)} runs")
    for seed in SEEDS:
    ...
    log(f"=== SQUEEZE2 LANE DONE {stamp()} === {(time.time() - started) / 60:.1f} min")
    return 0
