...
# dependencies = []
# ///
"""Resume the track-B training lanes after an interrupted run.

Stage B2 (evolve lane) and stage B4 (Doom-engine transfer lane) were killed
mid-flight. This runner replays only what is missing:
...
TRAIN_LOG = ROOT / "runs/doom/train.log"
...
BENCH_NAMES = (*(name for name, _, _ in EVOLVE_JOBS), "ppo")
...
def brain_path(level: str, seed: int, name: str) -> Path:
    return ROOT / f"runs/doom/{level}/seed{seed}/{name}.json"
...
def eval_csv(level: str, seed: int, name: str, eval_map: str) -> Path:
    return ROOT / f"runs/benchmark_doom/train_{level}/seed{seed}/eval_{eval_map}/{name}.csv"
...
def main() -> int:
    ...
    # ppo was trained by its own lane; make sure its simulator rows are complete too.
    for level in LEVELS:
        for seed in SEEDS:
            benchmark(level, seed, "ppo", SIM_MAPS)
