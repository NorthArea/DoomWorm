...
Это полигон. Его задача — понять, на что способен такой подход и насколько
он перспективен по сравнению с маленькой сетью, обученной с нуля.

Конечная цель за пределами полигона:

```text
физическая платформа (автомобиль)
      ↓
ездит по квартире
      ↓
изучает её и строит карту
      ↓
держит карту и перемещается сама
      ↓
в зависимости от потребностей: заряд, вызов, исследование
```

Роль червя в этой платформе — реактивный и мотивационный слой:
объезд препятствий, движение к цели по градиенту, исследование,
переключение режимов по потребности. Карта и планирование живут
вне мозга (см. §3.2).

Проект ведёт два равноправных трека на одной платформе:

```text
трек A   домашний пылесос   (основной, первый)
трек B   игрок в Doom       (второй, после платформы трека A)
```

В обоих треках порядок один: сначала мир, оснастка и эмуляции датчиков,
потом на готовой платформе сравниваются разные мозги (см. §3.3).
...
физический пылесос
+
мозг, выбранный по бенчмарку A2 (червь, сеть с нуля, RL или гибрид)
+
внешняя карта и планировщик
+
потребности: заряд, вызов, уборка
```

Пылесос должен:

```text
убирать квартиру (покрытие)
не застревать в стенах и мебели
возвращаться на док при низком заряде
строить карту и держать её
приезжать по вызову
```
...
инженерный слой. Трек B отвечает на тот же вопрос в другой среде.
