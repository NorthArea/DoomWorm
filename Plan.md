...
Doom остаётся опциональным треком (см. трек B): он полезен как
стресс-тест, но к домашнему роботу не приближает.
...
Трек B (Doom) начинается только после этой точки и только по желанию.
...
# 20.1. Трек A. Домашний робот

Основной трек после этапа 14. Каждый этап по-прежнему добавляет
одну сложность. Нумерация этапов продолжает §44.

---

# 20.2. Этап 15. Карты, похожие на квартиру

Генератор карт от seed:

```text
комнаты
двери
коридоры
мебель (прямоугольные препятствия)
```
...
Обученный на этапе 12 мозг проезжает через дверь в соседнюю комнату
на карте, которую не видел.

---

# 20.3. Этап 16. Батарея и док

Переименование с сохранением механики этапа 2:

```text
hunger → battery (1.0 полный, 0.0 разряжен)
food   → dock
```
...
```text
док не исчезает
зарядка занимает время (агент стоит на доке N тиков)
док излучает маяк: градиент как у еды, но с ограниченной дальностью
разряд батареи → конец эпизода, большой штраф
```

## Acceptance Criteria

Агент при низком заряде возвращается на док и заряжается; при
полном заряде уезжает с дока.

---

# 20.4. Этап 17. Шум и задержка датчиков

Симулятор приближается к железу:

```text
дальномеры: N лучей (по умолчанию 5: -60°, -30°, 0°, +30°, +60°)
дальность: 0.03 .. 2.0 м (по умолчанию, уточнить по железу)
шум: гауссов, σ пропорциональна расстоянию + выбросы (dropout)
задержка: 1-2 шага среды
одометрия: с накапливающейся ошибкой
```

Все параметры в одном конфиге датчиков, который потом заменяется
конфигом реальной машины.

## Acceptance Criteria

Мозг, обученный без шума, оценивается с шумом; мозг, обученный с шумом,
не хуже на чистой среде. Метрики сохраняются для §17.1.

---

# 20.5. Этап 18. Внешний слой карты

Occupancy grid вне мозга:

```text
показания дальномеров + одометрия
        ↓
occupancy grid (unknown / free / occupied)
```

Классический алгоритм, без обучения, без нейросетей. Мозг о карте
не знает.

## Acceptance Criteria

За N тиков исследования карта покрывает заданную долю квартиры;
ошибка карты относительно истинной геометрии ограничена.

---

# 20.6. Этап 19. Планировщик → виртуальный градиент

```text
цель (точка X)
        ↓
путь по occupancy grid (BFS / A*)
        ↓
направление к следующей точке пути
        ↓
Sensory Adapter: тот же канал, что food / target
        ↓
C. elegans ведёт локально, объезжает то, чего нет на карте
```

## Acceptance Criteria

Вызов «приехать в X» в другую комнату выполняется на незнакомой карте
после того, как карта построена (этап 18).

---

# 20.7. Этап 20. Потребности и их арбитраж
...
curiosity (неизвестные клетки карты → исследовать)
```

Каждая потребность — свой градиент. Арбитраж: какой градиент подаётся
червю сейчас. Приоритет по умолчанию:

```text
battery > call > curiosity
```

Арбитраж — простой автомат состояний вне мозга. Проверить отдельно,
может ли сам червь (через гейтинг вроде голода на этапе 2) выбирать
между двумя градиентами; это входит в вопрос §1.

## Acceptance Criteria

Агент на незнакомой карте: исследует, пока не изучит квартиру,
заряжается по необходимости, приезжает по вызову.

---

# 20.8. Этап 21. Физическая платформа

Только теперь железо. Первая машина — минимальная:

```text
differential drive
3-5 дальномеров (по конфигу этапа 17)
одометрия с колёс
контроллер класса Raspberry Pi / ESP32 (302 нейрона — это дёшево)
```

Меняются только Environment и адаптеры. Мозг — тот же JSON,
что и в симуляторе.

Порядок:

```text
1. ручное управление, запись датчиков → сверка с симулятором
2. этап 1 на железе: объезд препятствий
3. этап 16: возврат на док
4. этапы 18-20: карта, вызов, исследование
```

## Acceptance Criteria

Машина ездит по квартире, не врезается постоянно, возвращается на док,
строит карту, приезжает по вызову.

---

# 20.9. Трек B. Doom (опционально)

Всё ниже до §35 — опциональный трек. Начинается только после §17.1.
Этапы обозначаются B1-B12 и не входят в основной порядок §44.
...
батарея + док
   ↓
[17]
шум и задержка датчиков
   ↓
[18]
внешняя карта (occupancy grid)
   ↓
[19]
планировщик → виртуальный градиент
   ↓
[20]
...
Трек B (Doom), опционально, после §17.1:

```text
[B1] mini Doom → [B2] fire → [B3] combat → [B4] ViZDoom →
[B5..B11] Doom L1-L7 → [B12] visual input
```
...
+
карта квартиры, которую агент не видел
+
шумные датчики
+
батарея и док
+
внешняя occupancy grid и планировщик
+
вызов «приехать в X»
```

Агент в симуляторе исследует квартиру, заряжается и приезжает по вызову
(конец этапа 20).
...
физическая машина
+
differential drive, 3-5 дальномеров
+
тот же мозг (JSON), что обучен в симуляторе
+
внешняя карта и планировщик
+
потребности: заряд, вызов, исследование
```
...
```text
ездить по квартире
не застревать постоянно в стенах и мебели
возвращаться на док при низком заряде
строить карту квартиры и держать её
приезжать по вызову в указанную точку
исследовать неизученные части
```

Не требуется идеальная навигация. Требуется ответ на §1: что из этого
делает червь, что делает сеть с нуля, и что в любом случае делает
инженерный слой.
